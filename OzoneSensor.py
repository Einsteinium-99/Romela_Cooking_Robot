...
arr = np.zeros(1)
while(np.size(arr) < 10): #NEED TO CHANGE THIS CONDITION LATER
  ''' Smooth data collection the collection range is 1-100 '''
  arr = np.append(arr, ozone.get_ozone_data(COLLECT_NUMBER))
  print("Ozone concentration is %d PPB."%arr[np.size(arr) - 1])
  time.sleep(1)
arr = np.delete(arr, 0)
print(arr)

def createCSVfile():
    path_to_csv_file = '/home/pi/Desktop/DFRobot Ozone Sensor(0-10ppm)V1.0/OzoneData.csv'
